...
const EndedElections = ({ data }) => {
  ...
  const handleClick = (election) => {
    navigate(`/resultPage`, { state: { election } });
  };

  if (!data.length) {
    return (<div className="flex justify-center h-full font-medium opacity-70 text-xl">No ended Elections yet</div>);
  }
      ...
      {data.map((item, index) => (
        ...
        >
          <div className="flex flex-col flex-grow rounded-lg overflow-hidden border border-slate-400">
            ...
            <div className="h-12 bg-slate-800 flex-grow-0"></div>
          </div>
        </div>
      ))}
    </div>
